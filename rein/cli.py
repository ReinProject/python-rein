...
def post(multi, identity, defaults, dry_run):
    ...
    fields = [
        ...
        {'label': 'Mediator delegate address', 'value': mediator['Delegate signing address']},
...
def bid(multi, identity, defaults, dry_run):
    ...
    Pubkeys.set(rein,pubkey_for_escrow,privkey_for_escrow,userid=rein.user.id)
    Wallet.set(rein,primary_payment_address,primary_payment_privkey,ref=job['Job ID'],userid=rein.user.id)
...
def offer(multi, identity, defaults, dry_run):
    ...
    PersistConfig.set(rein,'next_addr_index',str(next_addr_index))
    Pubkeys.set(rein,pubkey_for_escrow,privkey_for_escrow,userid=rein.user.id)
    Wallet.set(rein,payment_address,payment_privkey,ref=bid['Job ID'],userid=rein.user.id)
...
def resolve(multi, identity, defaults, dry_run):
    ...
    worker_payment_daddr = doc['Primary worker payment address'];
    client_payment_daddr = doc['Primary client payment address'];
